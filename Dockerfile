...
RUN case "${TARGETARCH}" in amd64|arm64) ;; *) echo "Unsupported TARGETARCH: ${TARGETARCH}" >&2; exit 1 ;; esac; \
    ...
    git init -q /src/helm; git -C /src/helm remote add origin https://github.com/helm/helm.git; git -C /src/helm fetch -q --depth 1 origin "refs/tags/v${HELM_VERSION}:refs/tags/v${HELM_VERSION}"; git -C /src/helm checkout -q --detach "v${HELM_VERSION}"; test "$(git -C /src/helm rev-parse HEAD)" = "${HELM_COMMIT}"; make -C /src/helm build BINDIR=/out VERSION="v${HELM_VERSION}"; \
    git init -q /src/kubernetes; git -C /src/kubernetes remote add origin https://github.com/kubernetes/kubernetes.git; git -C /src/kubernetes fetch -q --depth 1 origin "refs/tags/v${KUBECTL_VERSION}:refs/tags/v${KUBECTL_VERSION}"; git -C /src/kubernetes checkout -q --detach "v${KUBECTL_VERSION}"; test "$(git -C /src/kubernetes rev-parse HEAD)" = "${KUBECTL_COMMIT}"; cd /src/kubernetes; version_ldflags="$(bash -c 'export KUBE_ROOT=/src/kubernetes; source hack/lib/version.sh; kube::version::get_version_vars; kube::version::ldflags')"; go build -trimpath -ldflags "${version_ldflags}" -o /out/kubectl ./cmd/kubectl; \
    git init -q /src/kustomize; git -C /src/kustomize remote add origin https://github.com/kubernetes-sigs/kustomize.git; git -C /src/kustomize fetch -q --depth 1 origin "refs/tags/kustomize/v${KUSTOMIZE_VERSION}:refs/tags/kustomize/v${KUSTOMIZE_VERSION}"; git -C /src/kustomize checkout -q --detach "kustomize/v${KUSTOMIZE_VERSION}"; test "$(git -C /src/kustomize rev-parse HEAD)" = "${KUSTOMIZE_COMMIT}"; cd /src/kustomize/kustomize; go build -trimpath -ldflags "-s -w -X sigs.k8s.io/kustomize/api/provenance.version=v${KUSTOMIZE_VERSION}" -o /out/kustomize .; \
    git init -q /src/yq; git -C /src/yq remote add origin https://github.com/mikefarah/yq.git; git -C /src/yq fetch -q --depth 1 origin "refs/tags/v${YQ_VERSION}:refs/tags/v${YQ_VERSION}"; git -C /src/yq checkout -q --detach "v${YQ_VERSION}"; test "$(git -C /src/yq rev-parse HEAD)" = "${YQ_COMMIT}"; cd /src/yq; go build -trimpath -ldflags "-s -w -X github.com/mikefarah/yq/v4/cmd.yqVersion=v${YQ_VERSION}" -o /out/yq .; \
